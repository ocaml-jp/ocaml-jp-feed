  ...
  | true ->
    let api_key =
      ...
      | None ->
        raise_s
          [%message
            "OPENROUTER_API_KEY is required to classify entries from a \
             mixed-content feed"
  ...
  let%map.Deferred.Or_error _final =
    Deferred.Or_error.List.fold entries ~init:state ~f:(fun state (entry : Entry.t) ->
      ...
      let%bind.Deferred.Or_error () =
        ...
        | None | Some true ->
          let%bind.Deferred.Or_error () = post_entry ~post_target ~feed_name:feed.name entry in
...
let process_feed ?replay_last ~post_target ~openrouter_api_key (feed : Feed_config.Feed.t) =
  ...
  | None, Some state ->
    let already_seen =
      String.Set.of_list (List.map state.seen ~f:Feed_state.Seen.id)
    in
    let new_entries =
